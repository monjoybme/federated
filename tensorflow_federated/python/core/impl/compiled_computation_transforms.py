...
class LambdaToCalledTupleOfSelectionsFromArg(transformation_utils.TransformSpec
  ...
  def should_transform(self, comp):
    return (
        isinstance(comp, computation_building_blocks.Lambda) and
        isinstance(comp.parameter_type, computation_types.NamedTupleType) and
        isinstance(comp.result, computation_building_blocks.Call) and
        isinstance(comp.result.function,
                   computation_building_blocks.CompiledComputation) and
        isinstance(comp.result.argument, computation_building_blocks.Tuple) and
        all(
            isinstance(x, computation_building_blocks.Selection)
            for x in comp.result.argument) and all(
                isinstance(x.source, computation_building_blocks.Reference)
                for x in comp.result.argument) and
        all(x.source.name == comp.parameter_name for x in comp.result.argument))

  def transform(self, comp):
    if not self.should_transform(comp):
      return comp, False
    if len(comp.result.argument.type_signature) > len(comp.parameter_type):
      raise ValueError('Inputs to TF computations cannot be masked.')
    parameter_names = [
